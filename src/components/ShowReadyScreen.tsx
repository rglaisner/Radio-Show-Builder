...
import { CheckCircle2, Play, TerminalSquare, ChevronDown, ChevronUp } from 'lucide-react';
...
export function ShowReadyScreen({
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-black text-white flex flex-col items-center justify-center p-6 overflow-hidden">
      ...
      <div className="w-full max-w-lg relative z-10 flex flex-col gap-6 max-h-full overflow-y-auto no-scrollbar">
        <div className="rounded-3xl bg-white/[0.04] border border-white/10 backdrop-blur-md shadow-2xl p-8 space-y-6 text-center">
        ...
        {showLog && logPanel ? (
          <div className="rounded-3xl bg-white/[0.02] border border-white/10 backdrop-blur-md overflow-hidden flex flex-col max-h-[40vh]">
            {logPanel}
          </div>
        ) : null}
      </div>
